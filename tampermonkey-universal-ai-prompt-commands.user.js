...
// @version      1.0.0
// @description  Tampermonkey skripta za zamjenu kratkih naredbi gotovim AI promptovima u AI chatovima.
...
(function () {
    ...
    const COMMANDS = {
        'HR1': `Prevedi celý poskytnutý text přesně do hrvatski. Sačuvaj význam, pořadí informací, jména, data, částky, čísla dokumentů, organizace a důležité formulace. Nepřidávej závěry, nezkracuj a neměň obsah.`,

        'HR2': `Sažmi poskytnutý text v hrvatskom podle významu a kontextu. Vysvětli téma, účastníky a hlavní obsah. Samostatně uveď požadavky, žádosti, rozhodnutí, data, lhůty, částky a důležité podrobnosti. Piš jednoduše a stručně.`,

        'HR3': `Izradi v hrvatskom krátké tematické shrnutí dopisu přesně na jeden řádek. Uveď odesílatele, téma, sdělení či požadavek a důležitá data, lhůty, částky, dokumenty nebo kroky.`,

        'HR4': `Prevedi poskytnutý text do jednoduché a srozumitelné němčiny na úrovni A2-B1. Text má být zdvořilý, úřední a gramaticky správný. Sačuvaj význam, data, jména, částky, adresy, organizace a podrobnosti.`,

        'HR5': `Oprav poskytnutý hrvatski text. Uprav jej gramaticky, srozumitelně a logicky, ale zachovej původní význam. Odstraň chyby, opakování a nevhodné formulace. U dopisu použij zdvořilý a úřední tón. Nepřidávej fakta.`,

        'HR6': `Napiši krátkou, zdvořilou a úřední odpověď na tento dopis v jednoduché němčině úrovně A2-B1. Odpověz věcně a správně formuluj potvrzení či žádosti o dokumenty nebo vysvětlení. Završi: Mit freundlichen Grüßen`,

        'HR7': `Vysvětli jednoduchými slovy v hrvatskom, co text znamená. Uveď, kdo píše, čeho se věc týká, co chce, co je třeba udělat a jaké jsou důležité lhůty, termíny, částky, dokumenty nebo podmínky. Urči, zda jde o požadavek, varování, rozhodnutí či informaci.`,

        'HR8': `Získej z textu všechny důležité skutečnosti a uspořádej je v hrvatskom: osoby, organizace, adresy, data, lhůty, částky, čísla dokumentů, požadavky, rozhodnutí, povinnosti, dokumenty a další kroky. Nic nevymýšlej; při chybějícím údaji napiš „neuvedeno“.`,

        'HR9': `Sestav v hrvatskom jasný seznam kroků podle textu. Uveď potřebné dokumenty, komu odpovědět, kam se obrátit, termíny a důležité body. Rozděl: naléhavé, důležité, může počkat. Uveď otázky k objasnění.`,

        'HR10': `Na základě textu napiš zdvořilý úřední dopis v jednoduché němčině úrovně A2-B1. Sačuvaj všechna důležitá fakta. Struktura: oslovení, situace, hlavní žádost nebo sdělení, případná žádost o potvrzení či vysvětlení, závěr. Završi: Mit freundlichen Grüßen`
    };
    ...
    function checkAndReplace(target) {
        ...
        replaceText(editable,COMMANDS[command]); showNotification("Naredba {cmd} zamijenjena je gotovim promptom".replace('{cmd}',command));
    }
